...
public class AabbUtil2 {
	...
	public static boolean rayAabb(Vector3d rayFrom, Vector3d rayTo, Vector3d aabbMin, Vector3d aabbMax, double[] param, Vector3d normal) {
		Vector3d aabbHalfExtent = Stack.alloc(Vector3d.class);
		Vector3d aabbCenter = Stack.alloc(Vector3d.class);
		Vector3d source = Stack.alloc(Vector3d.class);
		Vector3d target = Stack.alloc(Vector3d.class);
	...
	public static void transformAabb(Vector3d halfExtents, double margin, Transform t, Vector3d aabbMinOut, Vector3d aabbMaxOut) {
		Vector3d halfExtentsWithMargin = Stack.alloc(Vector3d.class);
		...
		Matrix3d abs_b = Stack.alloc(t.basis);
		MatrixUtil.absolute(abs_b);

		Vector3d tmp = Stack.alloc(Vector3d.class);

		Vector3d center = Stack.alloc(t.origin);
