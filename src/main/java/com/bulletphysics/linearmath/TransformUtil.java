...
//	#else
		...
		Vector3d axis = Stack.alloc(Vector3d.class);
		...
		Quat4d dorn = Stack.alloc(Quat4d.class);
		dorn.set(axis.x, axis.y, axis.z, (double) Math.cos(fAngle * timeStep * 0.5f));
		Quat4d orn0 = curTrans.getRotation(Stack.alloc(Quat4d.class));

		Quat4d predictedOrn = Stack.alloc(Quat4d.class);
...
// #else
		Matrix3d tmp = Stack.alloc(Matrix3d.class);
		tmp.set(transform0.basis);
		MatrixUtil.invert(tmp);

		Matrix3d dmat = Stack.alloc(Matrix3d.class);
