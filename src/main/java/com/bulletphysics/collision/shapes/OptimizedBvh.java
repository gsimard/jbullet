...
public class OptimizedBvh implements Serializable {
	...
	public void build(StridingMeshInterface triangles, boolean useQuantizedAabbCompression, Vector3d _aabbMin, Vector3d _aabbMax) {
		...
		if (useQuantization) {
			...
			Vector3d aabbMin = Stack.alloc(Vector3d.class);
			aabbMin.set(-1e30f, -1e30f, -1e30f);
...
//		}
	...
	public void updateBvhNodes(StridingMeshInterface meshInterface, int firstNode, int endNode, int index) {
		...
		Vector3d[] triangleVerts/*[3]*/ = new Vector3d[] { Stack.alloc(Vector3d.class), Stack.alloc(Vector3d.class), Stack.alloc(Vector3d.class) };
		Vector3d aabbMin = Stack.alloc(Vector3d.class), aabbMax = Stack.alloc(Vector3d.class);
		Vector3d meshScaling = meshInterface.getScaling(Stack.alloc(Vector3d.class));
...
//					break;
	...
	protected void walkStacklessQuantizedTreeAgainstRay(NodeOverlapCallback nodeCallback, Vector3d raySource, Vector3d rayTarget, Vector3d aabbMin, Vector3d aabbMax, int startNodeIndex, int endNodeIndex) {
		...
		//#define RAYAABB2
		//#ifdef RAYAABB2
		Vector3d rayFrom = Stack.alloc(raySource);
		Vector3d rayDirection = Stack.alloc(Vector3d.class);
...
//		boolean sign_z = rayDirection.z < 0f;
		...
		Vector3d rayAabbMin = Stack.alloc(raySource);
		Vector3d rayAabbMax = Stack.alloc(raySource);
		...
		Vector3d bounds_0 = Stack.alloc(Vector3d.class);
		Vector3d bounds_1 = Stack.alloc(Vector3d.class);
	...
	public void reportRayOverlappingNodex(NodeOverlapCallback nodeCallback, Vector3d raySource, Vector3d rayTarget) {
		...
		if (fast_path) {
			...
			Vector3d aabbMin = Stack.alloc(raySource);
			Vector3d aabbMax = Stack.alloc(raySource);
	...
	public void reportBoxCastOverlappingNodex(NodeOverlapCallback nodeCallback, Vector3d raySource, Vector3d rayTarget, Vector3d aabbMin, Vector3d aabbMax) {
		...
		if (fast_path) {
			...
			Vector3d qaabbMin = Stack.alloc(raySource);
			Vector3d qaabbMax = Stack.alloc(raySource);
