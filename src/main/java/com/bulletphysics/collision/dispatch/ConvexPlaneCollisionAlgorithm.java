...
public class ConvexPlaneCollisionAlgorithm extends CollisionAlgorithm {
	...
	public void processCollision(CollisionObject body0, CollisionObject body1, DispatcherInfo dispatchInfo, ManifoldResult resultOut) {
		...
		Vector3d planeNormal = planeShape.getPlaneNormal(Stack.alloc(Vector3d.class));
		...
		Vector3d vtx = convexShape.localGetSupportingVertex(tmp, Stack.alloc(Vector3d.class));
		Vector3d vtxInPlane = Stack.alloc(vtx);
		convexInPlaneTrans.transform(vtxInPlane);

		double distance = (planeNormal.dot(vtxInPlane) - planeConstant);

		Vector3d vtxInPlaneProjected = Stack.alloc(Vector3d.class);
		tmp.scale(distance, planeNormal);
		vtxInPlaneProjected.sub(vtxInPlane, tmp);

		Vector3d vtxInPlaneWorld = Stack.alloc(vtxInPlaneProjected);
		...
		if (hasCollision) {
			...
			Vector3d normalOnSurfaceB = Stack.alloc(planeNormal);
			planeObj.getWorldTransform(tmpTrans).basis.transform(normalOnSurfaceB);

			Vector3d pOnB = Stack.alloc(vtxInPlaneWorld);
